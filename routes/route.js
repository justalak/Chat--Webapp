...
module.exports= (app)=>{
    ...
    app.route('/read-all-message/:conv_id').put(messageController.readAllMessage);
    app.route('/read-message/:message_id').put(messageController.readMessageById);
    app.route('/get-users-seen/:message_id').get(messageController.getSeenUsers);
    app.route('/sendattachment/:conv_id/:user_send').post(messageController.addAttachment);
    app.route('/get-status/:message_id/:user_id').get(messageController.checkMessageStatus);
    //UpdateProfie
